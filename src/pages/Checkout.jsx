...
export default function Checkout() {
  ...
  const [dhlRate, setDhlRate] = useState(null);
  ...
  const shippingFee = isPhilippines ? (shipping?.final ?? null) : (dhlRate?.fee ?? null);
  ...
  const canPlace = contact.name && contact.email && contact.phone &&
    ...
    ) && confirmed && !hasStockViolations && !stockChecking;
  ...
  const handlePlace = async () => {
    ...
    const shippingAddress = isPhilippines
      ? `${address.street}, ${address.barangay ? address.barangay + ', ' : ''}${address.city}, ${address.province} ${address.postal_code}`.trim()
      : `${address.street}, ${address.city}, ${address.postal_code}, ${address.country}`.trim();
            ...
            <div className="space-y-3">
              ...
              {isPhilippines ? (
                ...
                <>
                  ...
                  <div>
                    <label className={LABEL}>Postal Code *</label>
                    <input value={address.postal_code} onChange={e => setAddress(a => ({ ...a, postal_code: e.target.value }))} className={INPUT} placeholder="SW1A 1AA" />
                  </div>
                  <div>
                    <label className={LABEL}>Delivery Notes</label>
                    <textarea value={address.notes} onChange={e => setAddress(a => ({ ...a, notes: e.target.value }))} rows={2}
                      className={`${INPUT} resize-none`} placeholder="Landmark, building, etc." />
                  </div>
                  {dhlRate && (
                    <div className="bg-[#0a0a0a] border border-[#ff8c00]/20 px-4 py-3 space-y-2">
                      ...
                      <div className="border-t border-[#1a1a1a] pt-2 space-y-1">
                        ...
                        <div className="flex justify-between font-mono-ui text-[10px] text-[#444] border-t border-[#1a1a1a] pt-1 mt-1">
                          <span>DHL Express (Zone {dhlRate.zone} · {dhlRate.roundedKg} kg)</span>
        ...
        <div className="space-y-4">
          ...
          <div className="border border-[#222] p-4 space-y-2">
            ...
            {shipping != null ? (
              ...
              <div className="flex justify-between font-mono-ui text-xs text-[#888]">
                <span>DHL Express Shipping</span><span>₱{dhlRate.fee.toLocaleString()}</span>
              </div>
            ) : (
              <div className="flex justify-between font-mono-ui text-xs text-[#888]">
          ...
          {!canPlace && !hasStockViolations && !stockChecking && (
