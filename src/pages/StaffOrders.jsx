...
import { CheckSquare, Square, RefreshCw, X, MapPin, Printer, CreditCard, Plus, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function StaffOrders() {
  ...
  const [completeModal, setCompleteModal] = useState(null); // { orderId, order }
  const [completionNote, setCompletionNote] = useState('');
  const [completing, setCompleting] = useState(false);

  // Inline tracking edit
  const [editTracking, setEditTracking] = useState({}); // { [orderId]: { logistics, tracking_number } }
  ...
  const loadOrders = async () => {
    setLoading(true);
    const o = await base44.entities.Order.list('-created_date', 200);
    setOrders(o);
    setLoading(false);
  };
  ...
  const handleCompleteOrder = async () => {
    ...
    setCompleting(true);
    await base44.functions.invoke('completeOrder', {
      orderId: completeModal.orderId,
      completionNote,
    });
  ...
  // Sort: Pending_Completion first, then rest
  const rawFiltered = filterStatus === 'All' ? orders : orders.filter((o) => o.status === filterStatus);
  const filtered = [
            ...
            <div className="card-tactical overflow-hidden">
              ...
              <div className="divide-y divide-[#1a1a1a]">
                {filtered.map((order) => {
                  ...
                  return (
                    <div key={order.id} className={`${selected.has(order.id) ? 'bg-[#ff8c00]/5' : ''} ${isPendingCompletion ? 'border-l-2 border-l-green-500' : ''}`}>
                      {/* Main row */}
                      ...
                      {/* Expanded: Shipping + Completion Tools */}
                        ...
                        <div className="px-4 py-4 bg-[#0d0d0d] border-t border-[#1a1a1a] space-y-4">
                          ...
                          {canComplete && (
                            <div className="border border-green-500/20 bg-green-500/5 p-4 space-y-3">
                              <div className="flex items-center gap-2">
                                {isPendingCompletion
                                  ? <><CheckCircle className="w-4 h-4 text-green-400" /><p className="font-mono-ui text-[10px] text-green-400">Customer has confirmed receipt — verify tracking before completing</p></>
