        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-[#1a1a1a]">
          {HIGHLIGHTS.map((h, i) => (
            <motion.div key={i} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
              ...
              <div className="flex-shrink-0 w-10 h-10 border border-[#ff6b00]/20 flex items-center justify-center bg-[#ff6b00]/5 group-hover:border-[#ff6b00]/50 transition-colors">
                <h.icon className="w-5 h-5 text-[#ff6b00]" />
              </div>
              <div>
                <p className="font-tactical text-2xl text-[#ff8c00] leading-none">{h.stat}</p>
