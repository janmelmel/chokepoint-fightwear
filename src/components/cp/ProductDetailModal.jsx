...
export default function ProductDetailModal({ product, onClose }) {
  ...
  const isSoldOut = product.stock_limit > 0 && (product.total_ordered || 0) >= product.stock_limit;
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            <button onClick={handleAdd} disabled={!canAdd}
              ...
              {added ? <><Check className="w-4 h-4" /> Added!</> :
               ...
               <><ShoppingBag className="w-4 h-4" /> {product.is_preorder ? 'Pre-order' : 'Add to Bag'}</>}
            </button>
          </div>
        </div>
      </motion.div>
